...
/* Durch setzen dieser Variable kˆnnen Sie entscheiden, welche Includes alle eingebunden werden sollen
...
int main()
{
    ...
    // Beim Beenden des Programms s‰mtliche Medien lˆschen
...
void fuelleDatenbank(std::vector<Medium*>& medien)
{
#if UNTERKLASSENVORHANDEN
    {
        ...
        neuesMedium = new DVD("Fluch der Karibik", 12, "Actionkomˆdie");
        medien.push_back(neuesMedium);
...
void mediumHinzufuegen(std::vector<Medium*>& medien)
{
    #if UNTERKLASSENVORHANDEN
    {
        ...
        // ¸ber std::cin.ignore(10,\n) wird die "Enter-Eingabe, die f¸r std::cin benˆtigt wird ignoriert
...
void mediumEntfernen(std::vector<Medium*>& medien)
{
    // ID des Mediums, welches gelˆscht werden soll
    ...
    // Suchen des richtigen Mediums zum Lˆschen
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id)
        {
            delete (*it);  // Objekt lˆschen
            medien.erase(it); // Zeiger auf das Objekt lˆschen
            success = true; // Lˆschen war erfolgreich
...
void mediumAusleihen(std::vector<Medium*>& medien, Datum aktuellesDatum)
{
    ...
    // Einlesen des Namens der Person, die das Medium ausleihen mˆchte
    std::string name;
